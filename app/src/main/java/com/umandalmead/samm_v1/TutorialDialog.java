...
public class TutorialDialog extends Dialog implements
    ...
    private TextView _TV_Instructions, _TV_Tutorial_Title;
    ...
    public TutorialDialog(Activity activity, String[] STR_array_Tutorials, Integer[] INT_array_ImageResources){
        super(activity);
    ...
    private void InitializeUIObjects(){
        try {
            ...
            setCancelable(false);
            SetTypeFace();
            if (_AL_INT_ResourceIds.size() != 0 && _AL_STR_Tutorials.size() != 0) {
                if(_AL_INT_ResourceIds.size() > 1 && _AL_STR_Tutorials.size() >1)
                    Toggle(_BTN_NEXT);
                else{
                    Toggle(_BTN_OK);
                }
    ...
    private void ViewNextStep(){
        try {
            if(_AL_INT_ResourceIds.size()-1 >= tutorialCtr && _AL_STR_Tutorials.size()-1 >= tutorialCtr) {
                _IV_tutorial_image.setImageResource(_AL_INT_ResourceIds.get(tutorialCtr));
                _TV_Instructions.setText(_AL_STR_Tutorials.get(tutorialCtr));
                if(_AL_INT_ResourceIds.size()-1 == tutorialCtr && _AL_STR_Tutorials.size()-1 == tutorialCtr){
                    Toggle(_BTN_OK);
